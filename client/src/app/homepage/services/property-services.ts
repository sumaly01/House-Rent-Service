...
export class PropertyService extends BaseService {
  ...
  getPropertyFiltered(filter) {
    ...
    return this.http.get(
      this.url +
        "all" +
        `?address=${filter.address}&propertyType=${filter.propertyType}&numberOfRooms=${filter.numberOfRooms}&maximum_budget=${filter.maximum_budget}&city=${filter.city}&parking=${filter.parking}`,
      this.getOptions()
    );
  }
