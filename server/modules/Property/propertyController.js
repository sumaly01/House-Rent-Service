...
propertyController.editProperty = async (req, res, next) => {
  try {
    ...
    if (property && property._id) {
      const update = await propertySch.findByIdAndUpdate(
        property_id,
        { $set: property },
        { new: true }
      );
...
propertyController.getAllProperty = async (req, res, next) => {
  try {
    ...
    if (propertyType) {
      searchQuery = {
        property_type: { $regex: propertyType, $options: "i" },
        ...searchQuery,
      };
    }
    if (numberOfRooms) {
      searchQuery = { ...searchQuery, numberOfRooms: { $gte: numberOfRooms } };
    }
    if (numberOfBathrooms) {
      searchQuery = {
        ...searchQuery,
        numberOfBathrooms: { $gte: numberOfBathrooms },
      };
    }
    if (maximum_budget) {
...
propertyController.getSingleProperty = async (req, res, next) => {
  ...
  try {
    var pulledData = await propertySch.findOne({
      _id: propertyId,
      is_active: true,
      is_deleted: false,
    });
    console.log("propertyyyy", pulledData);
    if (pulledData && pulledData._id) {
      ...
      return otherHelper.sendResponse(
        ...
        null,
        null,
        propertyConfig.errorIn.noProperty,
        null
      );
    }
  } catch (err) {
